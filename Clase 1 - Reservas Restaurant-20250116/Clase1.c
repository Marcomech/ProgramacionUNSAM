/*
 * CONSIGNA -  Gestión de Reservas de un Restaurante 
...
int main() {
    ...
    /* Lee el archivo  - acuerdense que hay que crear un archivo reservas.bin debido a la forma en la que lo abrimos */
    if (file == NULL)
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 3:
                ...
                while (fread(&reserva, sizeof(reserva), 1, file)) {
                    if (strstr(reserva, cliente) != NULL) {
                        encontrado = 1;
                        posicion = ftell(file) - sizeof(reserva); /* Guardar la posicion para sobrescribir */
                        break;
                    }
                };
                if (encontrado) {
                    ...
                    snprintf(reserva, sizeof(reserva), "%s - %s - %s", cliente, fecha, cantidad);
                    fseek(file, posicion, SEEK_SET); /* Salto a la posicionición para sobrescribir */
                    fwrite(&reserva, sizeof(reserva), 1, file);
            ...
            case 4:
                ...
                FILE *file_temp; /* Archivo Temporal para ir pasando los datos */

                file_temp = fopen("temp_reservas.bin", "wb"); /* Solo me interesa escribir */
                if (file_temp == NULL) {
                ...
                if (encontrado) {
                    if (remove("reservas.bin") != 0) { /* Elimino el archivo original */
                        perror("Error al eliminar el archivo original");
                        return 1;
                    }

                    if (rename("temp_reservas.bin", "reservas.bin") != 0) { /* Cambio el nombre del archivo temporal */
                    ...
                    /* 
                     * Si no encuentro la reserva, tengo que eliminar el archivo temporal.
                     * Otra opcion seria crear el archivo temporal UNICAMENTE si encuentro la reserva, pero tendria que leerlo 
                     * 2 veces. 
                     */
                ...
                file = fopen(filename, "rb+"); /* Dejo abierto el archivo para la proxima */
                if (file == NULL)
                    perror("Error al abrir el archivo");
